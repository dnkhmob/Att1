...
def readdata(req, a):
            ...
            cw_array[a].temp=req['main']['temp']
            cw_array[a].date=datetime.date.fromtimestamp(req['dt'])
            cw_array[a].humidity=req['main']['humidity']
            cw_array[a].pressure=req['main']['pressure']
...
try:
    while b < c:  
            ...
            readdata(requests.get(f'https://api.openweathermap.org/data/2.5/weather?q={cw_array[b].city}&appid={openweatherAPI}&units=metric'),b)
    ...
    print(f'Длительность последовательного выполнения задачи = {end_time-start_time}')              
except:
